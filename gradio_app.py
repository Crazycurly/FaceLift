...
class FaceLiftPipeline:
    ...
    def generate_3d_head(
        ...
        try:
            ...
            fxfycxcy_list = [
                [c["fx"], c["fy"], c["cx"], c["cy"]] for c in selected_cameras
            ]
            c2w_list = [np.linalg.inv(np.array(c["w2c"])) for c in selected_cameras]

            fxfycxcy = torch.from_numpy(
                np.stack(fxfycxcy_list, axis=0).astype(np.float32)
            )
            ...
            comp_image = (
                (comp_image.cpu().numpy() * 255.0).clip(0, 255).astype(np.uint8)
            )
            ...
            turntable_frames = render_turntable(
                gaussians, rendering_resolution=self.image_size, num_views=180
            )
            turntable_frames = rearrange(
                turntable_frames, "h (v w) c -> v h w c", v=180
            )
...
def main():
    ...
    if pipeline.examples_dir.exists():
        examples = [
            [str(f)]
            for f in sorted(pipeline.examples_dir.iterdir())
